...
<template>
    <div class="tw-flex tw-flex-col">
        ...
        <div class="t-input-text" :class="[colorscheme[theme]]" :classa="[inputDivClass, colorscheme[theme]]">
            <button type="button" class="tw-text-center tw-ml-1 tw-grid tw-content-center focus:tw-outline-none"
                :class="disabled ? 'hover:tw-cursor-not-allowed' : ''" v-if="leftIcon"
                @click="!disabled ? $emit('leftIconClick') : () => { }">
                <slot name="leftIcon">
                    <vue-feather :type="leftIcon" />
                </slot>
            </button>
            <input @mousedown="interactionStart" @keydown="interactionStart" @keyup="$emit('keyup')"
                @change="$emit('change')" :value="modelValue" @input="emitValue" :placeholder="placeholder"
                class="t-input tw-py-1.5 lg:tw-py-2 xl:tw-py-2.5' tw-px-1 tw-bg-transparent tw-outline-none min-w-full tw-flex-1"
                :class="inputClass" :readonly="readonly" :type="type" />
            <button type="button" class="tw-text-center tw-p-0.5 tw-content-center tw-grid focus:tw-outline-none"
                v-if="rightIcon" :class="disabled ? 'hover:tw-cursor-not-allowed' : ''"
