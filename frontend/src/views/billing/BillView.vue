...
<template>
    <main>
        <ModalDialog id="client-modal" v-model:show="productModalShow" title="Detalle de la factura">
        ...
        <ECard>
            ...
            <nav class="navbar">
                <div class="container-fluid" style="padding:0">
                    <EButton type="secondary" @click="go">+ Crear factura
                    </EButton>
                    <EButton type="secondary" @click="go">Notas de crédito
                    </EButton>
                    <ECol cols="9" md="6" xl="4">
                        <ListBox v-model="model" top-label="Seleccione un filtro" :options="templateList" />
                    </ECol>
                    <form class="d-flex" role="search">
                        <input class="form-control me-2" type="search" placeholder="Buscar factura"
                            aria-label="Search" />
            ...
            <Table :header="tableSettings">
                <template #body-cell="{ cellData, colIdx, rowIdx }">
                    <div v-if="colIdx > 5" class="tw-grid tw-grid-flow-col tw-rounded tw-overflow-hidden">
                        <button class="tw-bg-blue-600 tw-py-1 tw-text-white tw-mx-2 tw-rounded"
                            @click="showProduct(cellData as productModel)">
                            Ver más detalles
                        </button>
                        <button class="tw-bg-green-600 tw-py-1 tw-text-white tw-mx-2 tw-rounded" @click="go3">
                            Editar
                        </button>
