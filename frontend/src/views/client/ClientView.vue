...
<template>
    <main>
                ...
                <div class="row tw-pb-3 align-content-center justify-content-center gy-2">
                    <template v-for="(d, k) in detailSelectedItem.item" :key="k">
                        <div class="row" v-if="k == 'number_id'">
                            <span class="tw-w-1/2 tw-font-bold col-6">cédula:</span>
                        ...
                        <div class="row" v-else-if="k == 'address'">
                            <span class="tw-w-1/2 tw-font-bold col-6">dirección:</span>
                            <span class="col-6">{{ d }}</span>
                        </div>

                        <div class="row" v-if="k == 'email'">
                            <span class="tw-w-1/2 tw-font-bold col-6">email:</span>
                            <span class="col-6">{{ d }}</span>
                        </div>
                        <div class="row" v-if="k == 'phone_number'">
                            <span class="tw-w-1/2 tw-font-bold col-6">teléfono:</span>
                            <span class="col-6">{{ d }}</span>
                        </div>
                        <div class="row" v-else-if="k == 'city'">
                            <span class="tw-w-1/2 tw-font-bold col-6">ciudad:</span>
                            <span class="col-6">{{
                                    detailSelectedItem?.item?.city?.name
                            }}</span>
                        </div>
                        <div class="row" v-else-if="k == 'province'">
                            <span class="tw-w-1/2 tw-font-bold col-6">provincia:</span>
                            <span class="col-6">{{
                                    detailSelectedItem?.item?.province?.name
                            }}</span>
                        </div>
                        <div class="row" v-if="k == 'created_at'">
                            <span class="tw-w-1/2 tw-font-bold col-6">fecha de creación:</span>
                            <span class="col-6">{{ d }}</span>
                        </div>
                    </template>
                </div>
        ...
        <ECard>
            ...
            <ERow>
                <h1 class="tw-text-black first-line:dark:tw-text-neutral-300" style="font-size: 35px; color: black">
                    Clientes</h1>
            </ERow>
            <nav class="navbar">
            ...
            <WaitOverlay :show="showWaitOverlay">
                <BTable :fields="formFields" :items="form.items">
                    ...
                    <template #cell(Ciudad)="{ index }">{{ form.items[index]['city']?.name }}
                    </template>
