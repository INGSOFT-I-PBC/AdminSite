...
<template>
    <div class="tw-flex tw-flex-rows tw-justify-items-stretch tw-min-h-screen tw-w-full tw-justify-between tw-bg-neutral-100 dark:tw-bg-neutral-900 tw-overflow-x-hidden"
        id="_root">
        <<<<<<< Updated upstream <aside
            class="tw-h-full tw-min-w-fit tw-bg-slate-600 tw-rounded-r-xl tw-overflow-hidden tw-resize-x">
            <div id="__left-drawer"=======<aside
                class="tw-h-full tw-min-w-fit tw-bg-slate-600 tw-rounded-r-xl tw-overflow-hidden">
                <div id="__left-drawer">>>>>>> Stashed changes
                    class="tw-h-full tw-flex tw-flex-col tw-justify-self-stretch tw-justify-items-stretch
                    lg:tw-min-w-fit tw-resize-x">
                    <div id="__left-logo-container"
                        class="tw-p-10 tw-px-24 tw-bg-gray-300 tw-ring-slate-600 dark:tw-bg-slate-800">
                        <Title>LOGO</Title>
                    </div>
                    <div id="__left-menu-container" class="tw-w-full tw-flex-1 tw-overflow-y-auto">
                        <DrawerMenu ref="menuContainer">
                            <template v-for="(menuItemData, idx) of menuItems" :key="idx">
                                <DrawerMenuItem v-if="menuItemData.id != 'logout'" :data="menuItemData" />
                                <DrawerMenuItem v-else @click="logout" :data="menuItemData" />
                            </template>
                        </DrawerMenu>
                    </div>
                </div>
                ...
                <<<<<<< Updated upstream <div
                    class="resize-handle--x tw-cursor-ew-resize tw-justify-center tw-resize-x hover:tw-bg-primary-light tw-w-1 hover:tw-animate-pulse hover:tw-transition tw-ease-in-out tw-duration-700 tw-bg-transparent"=======<div
                    ...
                    <main class="main-section" id="right-panel">
                        <header class="tw-flex tw-flex-row tw-gap-3 tw-justify-between tw-justify-items-stretch">
                            <UserCard :username="authStore.userData?.username" :role="authStore.userData?.role" />
                            <div id="_page-title-container"
                            ...
                            <div id="_breadcrumb-container"
                                ...
                                <TransitionGroup enter-active-class="tw-transition-all tw-duration-400"
                                    ...
                                    <div v-for="(item, index) of breadcrumb" :key="index">
                                        <FontAwesomeIcon class="tw-h-4 tw-mr-2" :icon="`fa-solid ${
                                            index > 0
                                                ? 'fa-chevron-right'
                                        ...
                                        <RouterLink v-else class="hover:tw-text-primary hover:tw-font-bold"
                                            :to="item.href || '/'">
                        ...
                        <!-- End of Breadcrumb -->
                        <<<<<<< Updated upstream <!-- <div class="row tw-grid tw-z-10">-->
                            <Transition enter-active-class="tw-transition-all tw-duration-350 tw-ease-in"
                            ...
                            <footer class="tw-sticky tw-top-full tw-bottom-0 tw-pb-1 tw-pt-5 tw-z-0">
                                <span class="tw-text-neutral-500 dark:tw-text-neutral-400">Realizado en
