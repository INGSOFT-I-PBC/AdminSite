...
from api.serializers.invoices import (
    FullInvoiceSerializer,IProductStockSerializer,
    IClientSerializer,
...
class PaginatedItemInvoiceView2(ModelViewSet):
    pagination_class = ApiPagination
    queryset = ProductStockWarehouse.objects.all().order_by("-id").exclude(product__is_active=0)
    serializer_class = IProductStockSerializer
    ...
    def get_queryset(self):
        ...
        if params.get("buscar", None):
            queryset = queryset.filter(
                Q(variant__variant_name__icontains=params.get("buscar"))
                | Q(variant__sku__icontains=params.get("buscar"))
                | Q(product__product_name__icontains=params.get("buscar")) )
        return queryset
...
class InvoiceView(viewsets.GenericViewSet):
    ...
    def search_item(self, request: Request):
        ...
        supplier = ProductStockWarehouse.objects.filter(variant__pk=request.query_params.get("id")).first()
